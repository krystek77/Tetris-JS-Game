function Tetris(canvas) {
	...
	this.canvas.addEventListener('click', () => this.resetGame(event));
	this.update();
}
...
Tetris.prototype.drawButton = function() {
	...
	this.context.fillRect(this.canvas.width - 90, 10, 80, 40);
	this.context.strokeStyle = 'white';
	this.context.strokeRect(this.canvas.width - 90, 10, 80, 40);
	this.drawText('RESET', this.canvas.width - 75, 38, 'white', 'bold 20px sans-serif', 50);
};
...
Tetris.prototype.draw = function() {
	...
	this.drawText('LIFES', 60, this.canvas.height - 40, 'orange', 'bold 20px sans-serif', 40);
	this.drawText('00' + this.player.life, 60, this.canvas.height - 10, 'white', 'bold 40px sans-serif', 40);
	this.drawText('LEVEL', this.canvas.width - 100, this.canvas.height - 40, 'orange', 'bold 20px sans-serif', 40);
	this.drawText(
		this.player.level,
	...
	this.drawText('ROWS', 10, this.canvas.height / 2 + 10, 'white', 'bold 20px sans-serif', 40);
	this.drawText(this.player.rowCount, 10, this.canvas.height / 2 + 40, 'blue', 'bold 40px sans-serif', 40);
...
Tetris.prototype.resetGame = function(event) {
	...
	if (posX > this.canvas.width - 90 && posX < this.canvas.width - 90 + 80 && posY > 10 && posY < 10 + 40) {
		this.player.reset();
		this.player.life = 3;
		this.player.score = 0;
		this.player.level = 1;
